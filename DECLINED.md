...
| **A STREAM pty's last-seat write-end close is not serialized against a concurrent re-seat.** `tty_pty_unref` closes the slave's pipe write end (EOF to the master) when the seat count reaches zero; if a new task were seated (`tty_pty_ref`) in the window between the decrement observing zero and the close, that task would run with a live seat while the master already saw EOF and stopped reading its output. Serializing the zero-seat decision against refs means taking the pty list lock across the seat transitions on the task-teardown and spawn paths — locking a hot path to protect a state nothing reaches | Nothing re-seats a STREAM pty. husk holds its seat until it and every child it spawned have exited, so the count reaches zero only when the session is genuinely over, and telnetd seats exactly one husk and never a replacement. GRID ptys have no stream write end, so the race is STREAM-only. The window is real but no consumer can drive a task onto a stream slave whose seats have already emptied | A consumer RE-SEATS a stream pty — a terminal multiplexer, or a telnetd that reruns a shell on the same slave after the first exits. The day a second task is spawned onto a stream slave whose seats can hit zero under it, the transition needs the lock | `kernel/src/tty.c` `tty_pty_unref` (`stream_writer_closed`), `tty_pty_ref` |

...
// THE DOOR. `what` is a link, a submit control, or an implicit submission
// from a control (Enter in a box). Everything in families B-J runs here and
// nowhere else. The request owns its bytes; free it.
...
- **The 45 `expect_url` cases move in nearly verbatim** as the first
  families A-G cases; the 42 `expect_lines` cases stay with `wend`, because
  they test drawing.
...
- **libhtml** (Quinn): the parser form-owner pointer, when asked.

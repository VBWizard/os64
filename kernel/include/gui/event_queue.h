...
// Doorbells coalesce outside the ring and alternate with queued input when
// both are pending, so a busy worker cannot starve keys or close requests.
typedef struct gui_event_queue {
    ...
    uint32_t doorbell_mask;
    uint64_t doorbell_tick;
    bool doorbell_pending;
    bool doorbell_yield_input;
} gui_event_queue_t;

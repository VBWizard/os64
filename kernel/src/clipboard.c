...
// Logged under DEBUG_VFS on purpose: in slice 1 every byte of clipboard
// traffic arrives through /sys/clipboard, so this IS filesystem traffic and
// the tag does not lie. When the non-VFS consumers land (gterm's select-is-
// copy, the text VTs) a DEBUG_CLIPBOARD bit will have earned itself, and it
// gets added to CONFIG.h, klog_format.h's %g table, and log.c's static
// asserts together — the three places that must never disagree.
...
int clipboard_append(snarf_pending_t *pending, const void *bytes, size_t length)
{
	...
	if (need > CLIPBOARD_MAX_BYTES || need < pending->length /* overflow */)
	{
		...
		printd(DEBUG_VFS, "CLIPBOARD: refused a copy of %lu bytes (ceiling %lu) — "
	...
	if (need > pending->cap)
	{
		...
		if (grown == NULL)
		{
			printd(DEBUG_VFS, "CLIPBOARD: out of memory growing a copy to %lu bytes\n",
...
void clipboard_seal(snarf_pending_t *pending)
{
	...
	if (pending->poisoned)
	{
		printd(DEBUG_VFS, "CLIPBOARD: discarding a poisoned copy; the previous "
		       "snarf survives\n");
	...
	if (entry == NULL)
	{
		...
		printd(DEBUG_VFS, "CLIPBOARD: out of memory sealing a %lu byte copy\n",
	...
	printd(DEBUG_VFS, "CLIPBOARD: sealed %lu bytes at tick %lu\n",

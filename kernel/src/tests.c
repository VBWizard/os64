...
int testVFS(vfs_filesystem_t *testFS)
{
	...
	if(testFS->fops->open(&testFile, "/partition_info", "r", testFS)==0)
	{
        ...
        printd(DEBUG_TESTS, "/partition_info file contents via read = %s\n", contents);
        testFS->fops->seek(testFile, 0, SEEK_SET);
		memset(contents, 0, 4096);
		testFS->fops->fgets(testFile, contents, 4096);
        printd(DEBUG_TESTS, "/partition_info file contents via fgets = %s \n", contents);
        testFS->fops->close(testFile);
	}
	else
	{
        printd(DEBUG_TESTS, "File /partition_info NOT FOUND!\n");
        return -1;
	}
	...
	{
        printd(DEBUG_TESTS, "Failed to open root directory!\n");
        return -2;
	}
	...
	if (testFS->bops->write)
	{
		if (testFS->fops->open(&testFile, "/test2","c", testFS)==0)
		{
			...
			testFS->fops->close(testFile);
            printd(DEBUG_TESTS, "New test file /test2 written\n");
        }
		else
		{
            printd(DEBUG_TESTS, "Failed to create file /test2!\n");
            return -3;
		}
		...
		if(testFS->fops->open(&testFile, "/test2", "r", testFS)==0)
		{
			testFS->fops->fgets(testFile, contents, 4096);
			testFS->fops->close(testFile);
            printd(DEBUG_TESTS, "New file test2 Contents = %s\n", contents);
        }
		else
		{
            printd(DEBUG_TESTS, "Failed to read file test2 using fgets!\n");
            return -4;
		}
		...
		{
			if (lResult==FR_EXIST)
                printd(DEBUG_TESTS, "Directory /testdir already exists\n");

            if (testFS->fops->open(&testFile, "/testdir/testfile","c", testFS)==0)
			{
				testFS->fops->write(testFile, "Hello world from Chris too!\n",27);
				testFS->fops->close(testFile);
                printd(DEBUG_TESTS, "New test file /testdir/testfile written\n");
            }
			else
                printd(DEBUG_TESTS, "Failed to open file /testdir/testfile for create\n");

            if(testFS->fops->open(&testFile, "/testdir/testfile", "r", testFS)==0)
			{
				testFS->fops->fgets(testFile, contents, 4096);
				testFS->fops->close(testFile);
                printd(DEBUG_TESTS, "New file /testdir/testfile Contents = %s\n", contents);
            }
			else
                printd(DEBUG_TESTS, "Failed to open file /testdir/testfile for write\n");
        }
		else
		{
            printd(DEBUG_TESTS, "Failed to make directory /testdir\n");
            return -5;
		}
	}
	else

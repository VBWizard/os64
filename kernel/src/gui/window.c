    ...
    *retired=NULL;
    ...
    for (window_t *w=s_top;w;w=w->below) {
        ...
        if (!decoration_frame(w->frame,old,next,maximized,&frame) ||
            frame.w<(int32_t)os64_decor_min_width(view->header)) return false;
        ...
        if (cw>(int32_t)w->canvas_cap_w || ch>(int32_t)w->canvas_cap_h ||
            (maximized && (cw<(int32_t)w->min_content_w || ch<(int32_t)w->min_content_h))) return false;
        if (maximized && (!decoration_frame(w->restoreFrame,old,next,false,&restore) ||
            restore.w<(int32_t)os64_decor_min_width(view->header))) return false;
    }

...
{
	s->pixels = kmalloc_aligned((uint64_t)w * h * sizeof(uint32_t));
	if (!s->pixels) {
		printd(DEBUG_GUI, "surface_init: allocation failed for %ux%u surface\n", w, h);
		s->width = s->height = s->pitch_px = 0;
		return -1;
	}
	memset(s->pixels, 0, (uint64_t)w * h * sizeof(uint32_t));
	s->width = w;
	s->height = h;
	s->pitch_px = w;  // RAM surfaces are dense; only the hardware FB pads rows
	return 0;
}

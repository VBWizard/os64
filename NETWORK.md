...
| A transfer is slower than the wire and nothing else is wrong | `cat /sys/net/knet` (DOORBELL.md): `wakes` against the card's `rx_frames` says whether the drainer is waking once per frame (the expected shape without interrupt coalescing) or not at all (a NIC left tick-driven — `mode: tick-driven (NETPOLL)`, a missing MSI capability, a silent INTx probe); `budget_exhausted` climbing says one wake could not drain the flood; `wake_cycles_max` is the longest time the BSP spent in one drain. Upload one segment per tick (~134 KB/s) is the fingerprint of a drain that waits for the pass — the 2026-09-05 "before" number |

...
| Debt | Sev | Cost | Gate | Source |
|---|---|---|---|---|
| **Sequential-read latency: one disk command per fs block.** `ext2_read` issues one NVMe round trip per 1KB block (30MB log = ~30,000 polled commands ≈ the observed 10s; wc/cat both pay it; FAT bounded similarly by cluster-size runs). Fix #1, cheap: contiguous-run coalescing in `ext2_read` — walk bmap while blocks stay adjacent on disk, issue ONE `bops->read` for the whole run (mkfs lays files nearly contiguous, so runs are long). Fix #2, the arc: block cache + readahead (Chris's long-anticipated). The `READ_CHUNK_SIZE` bump (4KB→1MB, 2026-08-05) is the syscall-count half — real but ~10ms of the 10s; the round trips are the seconds | Perf | M (#1) / L (#2) | Chris's ruling 2026-08-05: wc alone doesn't rise to it — wait for a REAL sufferer | wc's 10-second os64.log read |
...
| **NVMe PRP-LIST path has never really run.** Every ext2 request is 1KB (≤2 pages ⇒ prp1/prp2 direct, no list); FAT's cluster runs rarely clear 8KB — so the `prpCount > 2` branch (build list, `free_prp_list`) is effectively untested code. The `READ_CHUNK_SIZE` 1MB bump may light it up via FAT multi-sector coalescing, and ext2 run-coalescing (row above) will make it load-bearing — TEST IT DELIBERATELY then (big aligned + unaligned reads, run lengths straddling the 2-page and MDTS boundaries) | Robustness | S | with the coalescing fix, or first big-read workload | the envelope autopsy, 2026-08-05 |

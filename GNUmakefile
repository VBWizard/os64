...
USERLAND_LIBS := userland/bin/libos64.so userland/bin/libgzip.so userland/bin/libpng.so userland/bin/libtls.so userland/bin/libjpeg.so userland/bin/libimage.so userland/bin/libfetch.so userland/bin/libhtml.so userland/bin/libpage.so
...
$(DISK_IMAGE): license/libpage-numeric-LICENSE license/libjpeg-turbo-LICENSE $(KERNEL_BIN) $(KERNEL_FIXTURES) $(USERLAND_BINS) $(USERLAND_TESTBINS) $(USERLAND_LIBS) kernel/test/partition_info.txt etc/husk.rc etc/desktop.conf etc/gclock.conf etc/os64.conf etc/gui.conf etc/bootenv.conf limine-hd.conf $(wildcard external/*) $(EXT2_TEST_IMAGE) GNUmakefile
...
# it available when the ext2 root is the thing being repaired.
	...
	# os64.conf, because the search path's root file must exist wherever a
	# root can be mounted. A lifeboat boot with no /etc/os64.conf still WORKS
	# (an absent file means the built-in default ladder), but the file is also
	# where you would edit your way out of trouble on the day root is broken,

...
typedef struct {
    ...
    bool (*headers_for)(void *ctx, const os64_url_t *url, bool encrypted,
                        char *out, size_t cap);
    void (*on_set_cookie)(void *ctx, const os64_url_t *url, bool encrypted,
...
`on_set_cookie` receives the originating URL, the actual connection's
`encrypted` flag, and the VALUE with surrounding space/tab removed. HTTPS
through a plain `$https_proxy` reports false even though the URL's scheme
is HTTPS. The libway session must use this flag for Secure-cookie acceptance;
libfetch delivers the original attribute without parsing or storing it.

The value excludes the field name, colon and CRLF. Repeated fields arrive
separately in wire order, including any comma in Expires. Delivery excludes 1xx responses and
...
`http_head_read` remains the no-callback wrapper. The
`http_head_read_one_with_headers` variant returns after one head, including
an interim reply, so uploads can resume between replies. Header and line
budgets remain `HTTP_HEAD_MAX`, `HTTP_HEADERS_MAX` and `HTTP_LINE_MAX`.

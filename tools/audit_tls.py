...
subprocess.run(['make', '-C', str(USERLAND), str(LIBRARY), str(FIXTURE)], check=True)
expected = set(re.findall(r'\b(os64_tls_\w+);', (BASE / 'exports.map').read_text()))
public = set(re.findall(r'\b(os64_tls_\w+)\(', (BASE / 'include/tls/tls.h').read_text()))
...
assert imports == {'os64_malloc', 'os64_free', 'os64_memcpy', 'os64_memmove', 'os64_memset',
                   ...
                   'os64_conf_find', 'os64_slurp'}, imports
...
print(f'TLS public ELF audit PASS: {len(expected)} exports, 12 OS imports, {len(manifest)} selected core sources')

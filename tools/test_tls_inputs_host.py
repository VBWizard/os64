...
def run(archive, work, public=False):
    ...
    if public:
        ...
        sources.append(BASE / "api.c")
        extra = ["-DTLS_TEST_PUBLIC", "-I" + str(work)]
    subprocess.run([os.environ.get("CC", "cc"), "-std=c11", "-O2", "-g", "-Wall", "-Wextra", "-Werror",
        ...
        *extra, *map(str, sources), str(BASE / "test/trust_vectors.c"), str(ROOT / "tools/test_tls_inputs_host.c"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--public", action="store_true", help="run through the public client entry points")
    ...
    def test(work):
        if not args.foundation:
            check(work)
        run(args.foundation.resolve() if args.foundation else work / "adapted/core.a", work, args.public)
    if args.output:
        args.output.mkdir()

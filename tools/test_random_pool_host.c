...
// jitter loop cannot; the variation check refusing a constant instruction;
// the retry budget counting; fast key erasure and the exact ChaCha20
// construction; the reseed cadence by bytes and by ticks; a contribution
// changing the key.
...
{
	// A little nonlinearity so the jitter loop's deltas vary when the step
	// does: real counters jitter, a straight line would fail the loop.
	g_cycles += g_cycle_step ? g_cycle_step + (g_cycles % 5) : 0;
	return g_cycles;
}

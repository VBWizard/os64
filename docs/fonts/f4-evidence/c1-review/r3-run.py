#!/usr/bin/env python3
"""Run the C1 third-review boundary probes against the current worktree.
...
out = args.output or Path(tempfile.mkdtemp(prefix='os64-f4-c1-r3-'))
...
subprocess.run(['cc', *flags, str(HERE/'r3-repro.c'), *map(str, sources+objects),
                '-o', str(out/'r3-repro')], check=True)
receipt = []
failed = False
for mode in ['caption', 'parent']:
    result = subprocess.run([str(out/'r3-repro'), mode,
                             str(ROOT/'userland/libfreetype/fixtures')],
                            env=env, stdout=subprocess.PIPE,
                            stderr=subprocess.STDOUT, text=True)
    (out/('r3-'+mode+'.txt')).write_text(result.stdout)
    receipt.append(f'{mode}: exit {result.returncode}\n{result.stdout}')
    print(receipt[-1], end='')
    failed |= result.returncode != 0
(out/'r3-observations.txt').write_text('\n'.join(receipt))

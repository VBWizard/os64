#!/usr/bin/env python3
"""Run the C1 fourth-review supplemental checks against the current worktree.
...
out = args.output or Path(tempfile.mkdtemp(prefix='os64-f4-c1-r4-'))
...
receipt = []
failed = False
for mode in ['supplemental']:
    result = subprocess.run([str(out/'r4-check'),
                             str(ROOT/'userland/libfreetype/fixtures')],
                            env=env, stdout=subprocess.PIPE,
                            stderr=subprocess.STDOUT, text=True)
    (out/('r4-'+mode+'.txt')).write_text(result.stdout)
    receipt.append(f'{mode}: exit {result.returncode}\n{result.stdout}')
    print(receipt[-1], end='')
    failed |= result.returncode != 0
(out/'r4-observations.txt').write_text('\n'.join(receipt))

...
| 4 | `read(handle, buf, len, timeout_ms)` | Dispatches on the tag: console keyboard (blocks), pipe read end (blocks; 0 = EOF = no writers left), or file (short near end, 0 AT end — same contract, zero file-awareness needed). Returns bytes read |

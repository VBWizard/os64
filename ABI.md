...
| 8 | `close(handle)` | Drop the handle. For pipe ends this is signalling (EOF/EPIPE refcounts); for files it closes (and may flush) the VFS file. THREE ANSWERS (`os64/file.h`): 0, the handle is gone and nothing was left undone; `OS64_CLOSE_NOT_COMMITTED` (-3), the handle is gone but what was written did not land — a FAT flush failed at close, or a file that judges what it is given (`/sys/console/font`) refused it; -1, no such handle (so a double-close is -1, never -3). The slot is freed before anyone can be told, which is why the middle answer is not "bad handle" |

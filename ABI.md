...
| 55 | `write_for(handle, buf, len, timeout_ms)` | TCP-only wait for send-ring space: 0 polls, finite milliseconds round up to ticks, `OS64_WAIT_FOREVER` waits for initial progress. Returns a queued prefix (at most 4096 bytes), `OS64_ERR_TIMEOUT` with no space by the deadline, or a write error. Ready space precedes timeout. A valid TCP handle accepts empty input with a NULL buffer; unsupported handles return -1. Timeout leaves the connection open. Syscall 3 keeps its existing three-argument ABI and blocking behavior. See [TCP write deadlines](TCP_WRITE_DEADLINE.md) |

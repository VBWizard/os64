...
| 9 | `open(path, mode)` | Open a file on the root fs, return a handle (≥ 3). mode: "r"/"w"/"a"/"c" for files, "d" for a DIRECTORY (readdir handle); NULL ⇒ "r"; validated at the boundary. File handles plug into read/write/seek/close AND spawn redirection (`prog < file` for free); dir handles plug into readdir/close only (spawn rejects them — no refcount, and a child's 0/1/2 are byte streams) |
...
| 43 | `rename(oldpath, newpath)` | Give a file a different name, possibly in another directory of the SAME filesystem. Replacing an existing REGULAR file is ATOMIC — the destination name never stops resolving, which is the entire reason rename exists (4.2BSD added it in 1983 to close link-then-unlink's window) and what makes write-a-temp-then-publish safe. Refused, never surprising: cross-filesystem (that's a copy — userland's job, Unix calls it EXDEV), a directory on either side of a replacement, either side held OPEN, or a directory moved into its own descendant. Atomicity is the filesystem's to grant: ext2 keeps it, FAT cannot (booked) |

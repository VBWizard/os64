...
| 9 | `open(path, mode)` | Open a file on the root fs, return a handle (≥ 3). mode: "r"/"w"/"a"/"c" for files, "u" = UPDATE an existing file in place (read+write, no truncate — hexedit's mode, 2026-08-22), "x" = atomically create a NEW file for writing and refuse an existing name, "d" for a DIRECTORY (readdir handle); NULL ⇒ "r"; validated at the boundary. File handles plug into read/write/seek/close AND spawn redirection (`prog < file` for free); dir handles plug into readdir/close only (spawn rejects them — no refcount, and a child's 0/1/2 are byte streams) |

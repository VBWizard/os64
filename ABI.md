...
| 3 | `write(handle, buf, len, timeout_ms)` | Dispatches on the handle tag. TCP honors finite patience: 0 polls, milliseconds round up to ticks, `OS64_WAIT_FOREVER` blocks. Queues while room exists; a full ring past the deadline returns progress or `OS64_ERR_TIMEOUT` if none. Nonempty writes on other handle types refuse finite waits. `os64_write` supplies forever; `os64_write_for` exposes the argument. See write's patience in [syscall_numbers.h](abi/include/os64/syscall_numbers.h) |
| 4 | `read(handle, buf, len, timeout_ms)` | Dispatches on the tag: console keyboard (blocks), pipe read end (honors poll/finite/forever patience; ready bytes and EOF precede timeout; 0 = EOF = no writers left), or file (short near end, 0 AT end — same contract, zero file-awareness needed). Returns bytes read |
| 5 | `spawn(path, argv, in, out, err)` | Launch child, return pid (non-blocking). in/out/err = caller's handles to install as child's 0/1/2, −1 = the caller's own 0/1/2 (the console at the console, its redirect when redirected). Stdio flows down; NO blanket handle inheritance, by design. Arguments are measured before they are copied: each under `OS64_SPAWN_ARG_MAX` (128 KiB), at most 512, a mebibyte in all with their pointers — past any of those the answer is `OS64_SPAWN_TOO_LONG` (−3), never a truncated argv |
...
| 8 | `close(handle)` | Drop the handle. For pipe ends this is signalling (EOF/EPIPE refcounts); for files it closes (and may flush) the VFS file. THREE ANSWERS (`os64/file.h`): 0, the handle is gone and nothing was left undone; `OS64_CLOSE_NOT_COMMITTED` (-3), the handle is gone but what was written did not land — a FAT flush failed at close, or a file that judges what it is given (`/sys/console/font`) refused it; `OS64_CLOSE_DEFERRED` (-5; not -4, which is `OS64_INTERRUPTED` system-wide and means "call again"), the handle is gone but another thread of the task was mid-operation on it, so the real close is that thread's and its outcome went to the log (a spawn on another thread with the handle as a redirection is such an operation until the child owns its copy; after that the copy answers for itself and the parent's close answers 0); -1, no such handle (so a double-close is -1, never -3). The slot is freed before anyone can be told, which is why the middle answers are not "bad handle" |
| 9 | `open(path, mode)` | Open a file on the root fs, return a handle (≥ 3). mode: "r"/"w"/"a"/"c" for files, "u" = UPDATE an existing file in place (read+write, no truncate — hexedit's mode, 2026-08-22), "x" = atomically create a NEW file for writing and refuse an existing name, "d" for a DIRECTORY (readdir handle); NULL ⇒ "r"; validated at the boundary. File handles plug into read/write/seek/close AND spawn redirection (`prog < file` for free); dir handles plug into readdir/close only (spawn rejects them — a child's 0/1/2 are byte streams, and a directory is not one) |
...
| 57 | `gui_window_set_min_size(handle, width, height)` | Owner-only minimum content dimensions; zero uses the default resize floor, limits must fit the reserved canvas, and a too-small window grows with a resize notification. See [WINDOW_MINIMUM_SIZE.md](WINDOW_MINIMUM_SIZE.md) |

                         ...
                         const os64_ui_theme_t *t)
{
	...
	os64_draw_rect(&ctx->surf, w->bounds, w->focused ? t->focus_ring :
	               w->hovered ? t->hover_border : t->button_border);
	// Bound relief to the actual control so tiny resized buttons cannot
	// paint their edges into a neighbor. Pressing reverses the light.
	...
	for (int32_t i = 1; i < bevel; i++) {
		int32_t x = w->bounds.x + i, y = w->bounds.y + i;
		int32_t width = w->bounds.w - 2 * i, height = w->bounds.h - 2 * i;
		os64_draw_hline(&ctx->surf, x, y, width, light);
		os64_draw_vline(&ctx->surf, x, y, height, light);
		os64_draw_hline(&ctx->surf, x, y + height - 1, width, dark);
		os64_draw_vline(&ctx->surf, x + width - 1, y, height, dark);
	}
	...
	if (ty < w->bounds.y)
		ty = w->bounds.y;
	os64_draw_text_clipped(&ctx->surf, w->bounds, tx, ty, w->text ? w->text : "", len,
	               t->button_fg, face);
}

...
// ── UTF-8 (str.h says who this is for) ──────────────────────────────────
...
size_t os64_utf8_decode(const char *s, size_t n, uint32_t *cp)
{
    ...
    if (lead >= 0xC2 && lead <= 0xDF) { need = 1; v = lead & 0x1F; }
        ...
        *cp = OS64_UTF8_REPLACEMENT;   // C0, C1, F5..FF, or a stray continuation byte
        return 1;
    }
    for (size_t i = 1; i <= need; i++) {
        if (i >= n || b[i] < lo || b[i] > hi) {
            *cp = OS64_UTF8_REPLACEMENT;   // truncated, or not a continuation byte
            return 1;
        }
        v = (v << 6) | (b[i] & 0x3F);
        lo = 0x80;

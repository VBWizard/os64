...
static void packet(ssh_engine *s, const uint8_t *p, size_t n)
{
    ...
    if (s->kex) {
         ...
         * Otherwise anything this state does not handle, known type or
         * not, earns UNIMPLEMENTED (RFC 4253 section 11.4), which is what
         * OpenSSH's kex_protocol_error answers: the rule names no message
         * numbers, so no range can be a number off. */
        if (s->strict && !s->established) ssh_disconnect(s, 2, "unexpected packet during strict key exchange");

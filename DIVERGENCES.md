...
| `rename(2)` of an open file succeeds | REFUSED while any handle holds either side open (in-band busy) | The same 2026-08-04 open-refcount ruling that governs unlink, applied to the verb that arrived twelve days later: a handle is a claim, and renaming out from under a reader is the same violation as deleting out from under one | ext2_write.c ext2_rename |
